  ...
  <div class="home-classify mui-ios mui-ios-10 mui-ios-10-3 mui-ios-10-3-1">
    ...
    <div class="content">
      ...
      <div class="left-wrapper" ref="leftScroll">
        <div>
          <div v-for="(l,i) in leftList" :key="i" @click="leftActive(i,$event)">
            <p v-text="l" :class="{'goods-active' : currentIndex == i}">左侧的选项</p>
          </div>
        </div>
      </div>
      <!-- 详细列表 -->
  ...
  methods: {
    ...
    leftActive(index, $event) {
      ...
      if (!$event._constructed) {
        ...
        //运用BScroll滚动到相应位置
        //运用index去找到对应的右侧位置
        let rightItem = this.$refs.rightScroll.getElementsByClassName(
          "wrapper"
        );
        //滚动到相应的位置

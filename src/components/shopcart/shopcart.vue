<template>
  <div>
    ...
    <div class="shop-cart-none" :class="{'d-none' : !shopcartHasSb}">
      ...
      <div><a :href="shopcart.href"><button class="button-blue" v-text="shopcart.btn">去逛逛</button></a></div>
    </div>
    <!-- 购物车商品 -->
    <div class="shopcart-list"  :class="{'d-none' : shopcartHasSb}">
      <div class="shopcart-top mui-checkbox mui-left">
         <input name="checkbox" value="Item 1" type="checkbox" @click="checkedAll" :checked="isCheckedAll"><span>魅族</span>
         <p>已免运费</p>
      </div>
      <div>
        <form class="mui-input-group">
          <div class="mui-checkbox mui-left " v-for="(item,index) in shopcartList" :key="index">
            <div class="shopcart-item"><img :src="item.url"><div>
                <h5 v-text="item.family_name"></h5><p v-text="item.spec"></p>
                <div><span v-text="'￥'+item.price"></span><a v-text="'x'+item.number"></a></div></div>
            </div>
    ...
    <div class="shopcart-footer">
      ...
      <div>
          <p> 总计:￥ <span v-text="totalMoney"> 56400.00</span></p>
      </div>
    </div>
  </div>
</template>
...
export default {
  data(){
      return  {
        shopcartHasSb:true,
        shopcart:{},
        outline:{msg:"登录后可显示您账号中已加入的商品",href:'#/login?url='+this.$route.path,btn:'去登录'},
        inline:{msg:"购物车内还没有商品，赶紧去选购吧～",href:'#/home',btn:'去逛逛'},
        shopcartList:[],
        checkedList:[],isCheckedAll:true, //选中状态
      }
  },
  components:{
        ...
        if(res.data.code == 1){
          this.$axios.post('/users/selectshopcart').then(result=>{
            ...
            if(result.data.code == 0){
              ...
              this.$store.commit('changeShopCart',result.data.msg.result.length)
  ...
  computed:{
    totalMoney(){ //总计
      var list = this.shopcartList , sum = 0
...
<style>
 ...
 .shopcart-footer div:first-child > input , .shopcart-footer div:first-child > h6{
   line-height: 50px;
   margin: 0 ;
 }
 .shopcart-footer div{
